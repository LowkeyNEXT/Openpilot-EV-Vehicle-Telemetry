...
one feature commit, so the implementation and its upstream delta remain easy to
inspect.
...
- an authenticated `GET /api/vehicle/telemetry` endpoint that remains available
  while driving;
- cache-only, LAN, personal Tailscale Funnel, and self-hosted FRP modes;
- optional bounded HTTPS push with independent credentials and cadence;
...
1. Install this branch as a full openpilot fork. Comma's standard
   ...
   redirect. If it is published only as `<owner>/EV-Vehicle-Telemetry`, use the
   full repository URL over SSH as shown below.
...
4. Choose **Tailscale** for a stable public HTTPS URL owned by your personal
   Tailscale account, **This Wi-Fi only** for LAN access, or **No network** for
...
git clone --recurse-submodules --shallow-submodules -b main \
  https://github.com/<owner>/EV-Vehicle-Telemetry.git openpilot
sudo reboot
```
...
when setup finishes, or when the vehicle starts. The low-priority daemon and
authenticated read-only API continue to run onroad.
...
| Mode | Behavior |
| --- | --- |
| `off` | cache only; optional HTTPS push can still run |
